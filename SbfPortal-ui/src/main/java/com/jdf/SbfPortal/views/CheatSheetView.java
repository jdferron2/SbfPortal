...
public class CheatSheetView extends HorizontalLayout implements View {
	...
	private final Command filterCommand = new Command() {
		...
		private void buildView(){
			...
			grid = new Grid<>(UserSessionVars.getRankSet().getRankSetName());
			grid.setSizeFull();
			...
			layout.addComponents(grid,upDownLayout,createNewListLayout);
		...
		private HorizontalLayout getCreateNewListLayout(){
			...
			Button createCustom = new Button("Create Custom Cheatsheet");
			createCustom.addStyleName(ValoTheme.BUTTON_LINK);
			createCustom.addClickListener(new Button.ClickListener()
			{ @Override public void buttonClick(Button.ClickEvent clickEvent)
			{
				UI.getCurrent().addWindow(getCreateNewListWindow());
			} });
			l.addComponent(createCustom);
			return l;
		}
		...
		private Window getCreateNewListWindow(){
			...
			if(UserSessionVars.getCurrentLeague() != null){
				makeDefault.setCaption("Make this default for " + UserSessionVars.getCurrentLeague().getLeagueName());
			...
			{ @Override public void buttonClick(Button.ClickEvent clickEvent)
			{
				if (listName.getValue() != null && !listName.getValue().equals("")){
					...
					leagueMgr.refreshCheatsheetToDefault(newRankSet);
